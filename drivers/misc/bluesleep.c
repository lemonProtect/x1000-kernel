...
static irqreturn_t bluesleep_hostwake_isr(int irq, void *dev_id)
{
	...
	BT_SLEEP_DBG("%s,cljiang-----trigger\n", __func__);
...
void bluesleep_stop(void)
{
        ...
        /* assert BT_WAKE */
//        gpio_set_value(bsi->ext_wake, 0);  config is converse
          gpio_set_value(bsi->ext_wake, 0);  //promise ext_wake pin being level on power down state
        del_timer(&tx_timer);
                                            ...
                                            size_t count, loff_t *offset)
{
		...
		if (buf[0] == '0') {
				gpio_set_value(bsi->ext_wake, 0);
				BT_SLEEP_DBG("bsi->ext_wake, 0\n");
        } else if (buf[0] == '1') {
                gpio_set_value(bsi->ext_wake, 1);
				BT_SLEEP_DBG("bsi->ext_wake, 1 \n");
        } else {
              kfree(buf);
              return -EINVAL;
        }
...
//	struct bt_rfkill_platform_data *gpio_data = pdev->dev.platform_data;

	printk("cljiang---bt_power_suspend\n");
//	gpio_set_value(bt_wake,0);
	if(1 == bt_power_state){
//		outl(gpio_data->gpio.bt_int_bit, gpio_data->gpio.bt_int_flagreg); /*clear bt_int irq flag*/
//		enable_irq(bsi->host_wake_irq);
		gpio_direction_output(bt_uart_rts,1);
	}
/*	if (gpio_data->set_pin_status != NULL){
	(*gpio_data->set_pin_status)(bt_power_state);
	}else{
		BT_SLEEP_DBG("set_pin_status is not defined\n");
	}
*/
//#endif
	return 0;
...
/*
	if (pdata->restore_pin_status != NULL){
		(*pdata->restore_pin_status)(bt_power_state);
	}else{
		BT_SLEEP_DBG("restore_pin_status is not defined\n");
	}
*/
...
static int /*__init*/ bluesleep_probe(struct platform_device *pdev)
{
	...
	BT_SLEEP_DBG("bluesleep probe mallac bluesleep_info done\n");
		...
		if(!pdata){
			BT_SLEEP_DBG("Can not find data about bt_rfkill_platform_data\n");
			return -ENODEV;
		}
...
static int __init bluesleep_init(void)
{
	...
	INIT_WORK(&hostwake_work, bluesleep_hostwake_work);
        ...
        /* assert bt wake */
   //     gpio_set_value(bsi->ext_wake, 0);   config is converse
	gpio_set_value(bsi->ext_wake, 0); //promise ext_wake pin being level on init status
#ifndef CONFIG_BT_BLUEDROID_SUPPORT
